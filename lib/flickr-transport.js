...
FlickrTransport.prototype.oAuth = function (params, isUpload) {
	...
	return function (req) {
		...
		if (isUpload) {
			...
			req.field('oauth_signature', flickrTransport.createSignature(req, merge(params, oauthParams), isUpload));
			...
			req.query({
				oauth_signature: flickrTransport.createSignature(req, merge(params, oauthParams), isUpload)
			});
...
FlickrTransport.prototype.call = function (definition, params, additionalParams) {
	...
	return new Promise(function (resolve, reject) {
		...
		function makeRequest() {
			...
			.use(flickrTransport.oAuth(oauthParams, apiParams.photo))
