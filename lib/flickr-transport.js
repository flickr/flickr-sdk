 ...
 */
FlickrTransport.prototype.createSignature = function (req, params, isUpload) {

	var baseString = req.method.toUpperCase() + '&' + encodeURIComponent(req.url) + '&',
	    signingKey = this.config.apiSecret + "&" + (this.config.accessTokenSecret || '');
 ...
 * @returns {Function} - A callable function that takes the request as a parameter.
 */
FlickrTransport.prototype.oAuth = function (params, isUpload) {
	...
	return function (req) {
		...
		if (isUpload) {
			...
			var oauthParams = {
				...
				oauth_token:            flickrTransport.config.accessToken,
				oauth_version:          '1.0',
			...
			req.field('oauth_signature', flickrTransport.createSignature(req, merge(params, oauthParams)));
			...
			req.query({
				oauth_signature: flickrTransport.createSignature(req, merge(params, oauthParams))
			});
 ...
 * @returns {Promise}
 */
FlickrTransport.prototype.call = function (definition, params, additionalParams) {
	...
	return new Promise(function (resolve, reject) {
		...
		var uri = generateURL(definition, apiParams, flickrTransport.config);
		...
		function makeRequest() {
			...
			.use(function (request) {
				...
				// to the request
				if (flickrTransport.config.accessToken) {
					return flickrTransport.oAuth(oauthParams, apiParams.photo)(request);
				}
				return request;
			})
		...
		function handleRequest(err, res) {
			...
			if (err) {
				...
				if (res.type === 'text/plain' && res.req.path.indexOf('/oauth/request_token') > 0) {
					var values = (res.text || '').split('&');
					values.forEach(function (keyPair) {
						var keyVal = keyPair.split('=');
						if (keyVal.length > 1) {
							res.body[keyVal[0]] = keyVal[1];
						}
					})
				}
